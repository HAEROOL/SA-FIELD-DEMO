...
interface TrendingBoardListProps {
  currentBoard?: string;
}

export default function TrendingBoardList({ currentBoard = "popular" }: TrendingBoardListProps) {
  const [page, setPage] = useState(0);
  const [isPending, startTransition] = useTransition();

  const { data: trendingData } = useTrendingPosts(page, 15);

  const posts = trendingData?.posts.content || [];
  const notices = trendingData?.notices.content || [];
  const totalPages = trendingData?.posts.totalPages || 1;
